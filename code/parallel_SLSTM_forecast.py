...
if __name__ == '__main__':
    ...
    # Here fill-in a baseline forecasts for comparison
    rate_df = DataFrame()
    ...
    inv_truth_v3_4wk = []
    mae_list_v3_4wk = []
    # Change you last date for training period
    train_date = datetime(2022, 8, 6)

    # Run 9-times for ensemble
    # Note: Split into shorter models might improve accuracy,
    # but will harm interpretation. There might have gaps between each model.
    # One-shot for longer period model can solve this problem, but might not
    # as accurate as shorter-period ensemble models.
    # Please balance your own ensemble strategy.

    # 4*weekly models: 3 times
    # 2*bi-weekly models: 3 times
    # 1*monthly models: 3 times
    for n in range(1, 10):
        if n in [1, 2, 3]:
            n_period = 4
            n_out = 8 * 1
        ...
        for i in range(n_period):
            ...
            ################ Save Model ################
            model.save(
                f"/content/gdrive/MyDrive/COVID-Hub_forecasts/{str(train_date.date())}/{str(train_date.date())}_week{i + 1}_model_{n}")
            ################ Huber loss ################
